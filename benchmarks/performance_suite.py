...
class PerformanceBenchmarker:
    ...
    async def benchmark_async(
        ...
        **kwargs
        ...
        # Determine iteration count
        if iterations is None:
            iterations = await self._calibrate_iterations_async(coro_func, *args, **kwargs)
...
async def run_cryptographic_benchmarks(benchmarker: PerformanceBenchmarker) -> List[BenchmarkResult]:
    ...
    for size in [64, 256, 1024, 4096, 16384]:
        ...
        result = benchmarker.benchmark(
            f"blake3_hash_{size}b",
            lambda d=data: blake3.blake3(d).hexdigest()
        )
    ...
    result = benchmarker.benchmark(
        "sha3_256_1kb",
        lambda: hashlib.sha3_256(data_1kb).hexdigest()
    )
    results.append(result)
    ...
    result = benchmarker.benchmark(
        "ed25519_verify",
        lambda: public_key.verify(signature, message)
    )
    results.append(result)
...
async def run_ihsan_benchmarks(benchmarker: PerformanceBenchmarker) -> List[BenchmarkResult]:
    """Benchmark Ihsān validation operations."""
    from ihsan_bridge import IhsanScore, WEIGHTS
    ...
    result = benchmarker.benchmark(
        "ihsan_weights_sum",
        lambda: abs(sum(WEIGHTS.values()) - 1.0) < 1e-10
    )
    results.append(result)
...
async def run_verification_benchmarks(benchmarker: PerformanceBenchmarker) -> List[BenchmarkResult]:
...
async def run_value_oracle_benchmarks(benchmarker: PerformanceBenchmarker) -> List[BenchmarkResult]:
...
async def run_batch_verification_benchmarks(benchmarker: PerformanceBenchmarker) -> List[BenchmarkResult]:
...
async def run_memory_layer_benchmarks(benchmarker: PerformanceBenchmarker) -> List[BenchmarkResult]:
...
def main():
    ...
    for r in sorted(suite.results, key=lambda x: x.p99_ms):
        print(f"{r.name:<40} {r.mean_ms:<12.4f} {r.p99_ms:<12.4f} {r.ops_per_second:<12.0f}")
