...
async def setup_demo_knowledge_graph(l4: L4SemanticHyperGraphV2):
    ...
    print(f"   Interdisciplinary Ratio: {topology.get('interdisciplinary_ratio', 0.0):.2%}")
...
async def run_graph_of_thoughts_demo(l4: L4SemanticHyperGraphV2):
    ...
    result = await process_with_graph_of_thoughts(
        observation,
        l4,
        enable_got=True
    )
...
async def main():
    ...
    print("║" + " "*8 + "Interdisciplinary Reasoning with SNR Optimization" + " "*10 + "║")
    print("╚" + "="*68 + "╝")
    ...
    try:
        ...
        print(f"   You can start Neo4j with: docker run -p 7687:7687 -p 7474:7474 neo4j")
