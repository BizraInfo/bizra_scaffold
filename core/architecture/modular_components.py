...
class CognitiveProcessor:
    ...
    def _generate_action(
        self, obs: Observation, clarity: float, synergy: float
    ) -> Dict[str, Any]:
        return {
...
class VerificationCoordinator:
    ...
    async def verify(
        ...
        tier, max_latency = self.TIER_THRESHOLDS.get(
            observation.urgency, ("L2_MERKLE", 50)
        )
        ...
        if tier == "L1_HASH":
            ...
            proof_hash, verified = await self._l2_merkle_verify(
                observation, convergence
            )
        elif tier == "L3_ZK":
            proof_hash, verified = await self._l3_zk_verify(observation, convergence)
        else:
            proof_hash, verified = await self._l4_formal_verify(
                observation, convergence
            )
    ...
    async def _l4_formal_verify(
        ...
        return proof, conv.quality in (
            ConvergenceQuality.OPTIMAL,
...
class ValueAssessor:
    ...
    async def assess(self, convergence: ConvergenceResult) -> ValueResult:
        ...
        composite = sum(self._weights.get(k, 0.0) * v for k, v in oracle_scores.items())
...
class EthicsEvaluator:
    ...
    async def evaluate(
        self, convergence: ConvergenceResult, observation: Observation
    ) -> EthicsResult:
        ...
        permissible = (
            convergence.clarity > 0.3
            and convergence.synergy > 0.4
            and severity_score < 0.6
        )
...
def create_custom_orchestrator(
    ...
    cognitive = (
        CognitiveProcessor(**convergence_config)
        if convergence_config
        else CognitiveProcessor()
    )

    value = ValueAssessor(weights=ihsan_weights)

    return UltimateOrchestrator(cognitive_processor=cognitive, value_assessor=value)
