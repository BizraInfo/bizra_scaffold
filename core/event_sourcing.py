...
class EventStore:
    ...
    async def get_events_by_time_range(
        ...
        return [e for e in events if start <= e.metadata.timestamp <= end]
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "events_by_type": {k: len(v) for k, v in self._by_type.items()},
        }
...
class AgentSummaryProjection(Projection):
    ...
    def get_high_ihsan_agents(self, threshold: float = 0.98) -> List[Dict[str, Any]]:
        ...
        return [s for s in self._summaries.values() if s["ihsan_score"] >= threshold]
