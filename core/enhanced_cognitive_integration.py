...
from core.snr_scorer import SNRLevel, SNRMetrics, SNRScorer, SNRThresholds
from core.tiered_verification import ConvergenceResult, QuantizedConvergence
from core.ultimate_integration import Observation, UltimateResult
...
class EnhancedCognitiveProcessor:
    ...
    async def process(self, observation: Observation) -> EnhancedCognitiveResult:
        ...
        perception_snr = self.snr_scorer.compute_from_convergence(
            ...
            disagreement=disagreement
            * 0.8,  # Perception typically has less disagreement
            ihsan_metric=ihsan_metric,
        )
        ...
        narrative = self.narrative_compiler.compile(
            synthesis, style=NarrativeStyle.TECHNICAL
        )
        ...
        self.metrics.record_snr_metrics(
            overall_snr.snr_score, overall_snr.level.name, "overall"
        )
    ...
    async def _safe_compute_convergence(
    ...
    async def _fallback_convergence(
        self, observation: Observation
    ) -> ConvergenceResult:
    ...
    async def _get_disagreement_from_oracle(
        self, observation: Observation
    ) -> Optional[float]:
        """
        ...
        try:
            ...
            logger.debug(
                f"Value oracle disagreement: {assessment.disagreement_score:.4f}"
            )
    ...
    async def _get_ihsan_from_ethics(self, observation: Observation) -> Optional[float]:
        ...
        try:
            ...
            action = Action(
                ...
                intended_outcome=observation.context.get(
                    "intent", "Knowledge synthesis"
                ),
            )

            context = EthicsContext(
                stakeholders=observation.context.get(
            ...
            if ihsan_score is None:
                logger.warning(
                    "IhsanEvaluator not found in verdict, using overall score"
                )
    ...
    async def _create_enhanced_synthesis(
        ...
        synthesis = CognitiveSynthesis(
            ...
            interdisciplinary_consistency=self._compute_interdisciplinary_consistency(
                thought_chains
            ),
            ...
            thought_graph_metrics={
                ...
                "domain_diversity": (
                    thought_chains[0].domain_diversity if thought_chains else 0.0
                ),
