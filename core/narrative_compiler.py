...
class CognitiveSynthesis:
    ...
    domain_bridges: List[Dict[str, Any]] = field(
...
class TechnicalNarrativeTemplate(NarrativeTemplate):
        ...
        if synthesis.ihsan_scores:
            sections.append(
                NarrativeSection(
                    ...
                    supporting_evidence=[
                        f"{dim}: {score:.2f}"
                        for dim, score in synthesis.ihsan_scores.items()
                    ],
                )
            )
    ...
    def _generate_technical_summary(self, synthesis: CognitiveSynthesis) -> str:
        ...
        if synthesis.snr_scores or synthesis.thought_graph_metrics:
            ...
            if synthesis.domain_bridges:
                enhancement += (
                    f" {len(synthesis.domain_bridges)} interdisciplinary bridges."
                )
    ...
    def _generate_action_caveats(self, synthesis: CognitiveSynthesis) -> List[str]:
        ...
        if synthesis.quantization_error > 0.01:
            caveats.append(
                f"Elevated quantization error ({synthesis.quantization_error:.4f})"
            )
    ...
    def _extract_ethical_evidence(self, synthesis: CognitiveSynthesis) -> List[str]:
        ...
        if "evaluations" in verdict:
            for eval in verdict["evaluations"][:3]:
                evidence.append(
                    f"{eval.get('framework', 'Unknown')}: {eval.get('score', 0):+.2f}"
                )
        return evidence
...
class ExecutiveNarrativeTemplate(NarrativeTemplate):
    ...
    def _format_risk(self, synthesis: CognitiveSynthesis) -> str:
        risk_level = (
            ...
            else "MEDIUM" if synthesis.confidence >= 0.5 else "HIGH"
        )
    ...
    def _format_recommendation(self, synthesis: CognitiveSynthesis) -> str:
        if synthesis.confidence >= 0.8 and synthesis.ethical_verdict.get(
            "permitted", True
        ):
    ...
    def _generate_executive_summary(self, synthesis: CognitiveSynthesis) -> str:
        status = (
            "APPROVED"
            if synthesis.ethical_verdict.get("permitted", True)
            and synthesis.confidence >= 0.7
...
class ConversationalNarrativeTemplate(NarrativeTemplate):
    ...
    def _explain_confidence(self, synthesis: CognitiveSynthesis) -> str:
        ...
        if pct >= 90:
            ...
            return (
                f"The system has low confidence ({pct:.0f}%). This needs human review."
            )
    ...
    def _generate_conversational_summary(self, synthesis: CognitiveSynthesis) -> str:
        conf = (
            "high"
            if synthesis.confidence >= 0.7
            else "moderate" if synthesis.confidence >= 0.5 else "low"
        )
        ...
        if synthesis.ihsan_scores:
            sections.append(
                NarrativeSection(
                    ...
                    supporting_evidence=[
                        f"{k.upper()}: {v:.4f}"
                        for k, v in synthesis.ihsan_scores.items()
                    ],
                )
            )
...
class NarrativeCompiler:
    ...
    def compile_multi_style(
        self, synthesis: CognitiveSynthesis
...
def self_test():
    ...
    print(
        f"✓ Technical narrative: {len(tech_narrative.sections)} sections, "
    ...
    print(
        f"✓ Executive narrative: {exec_narrative.reading_time_seconds}s read time, "
    ...
    print(
        f"✓ Conversational narrative: complexity={conv_narrative.complexity_score:.2f}"
    )
    ...
    ihsan_section = next(
        (s for s in tech_narrative.sections if "Ihsān" in s.title), None
    )
