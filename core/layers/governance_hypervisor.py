...
try:
    from core.layers.blockchain_substrate import (
        BlockchainSubstrate, BlockType, IhsanEnforcer
    )
...
class FATEMetrics:
    ...
    def validate(self, threshold: float = 0.95) -> Tuple[bool, str]:
        ...
        if self.transparency < threshold:
            return (False, f"Transparency violation: {self.transparency:.4f} < {threshold}")
        if self.empowerment < threshold:
            return (False, f"Empowerment violation: {self.empowerment:.4f} < {threshold}")
        if self.composite < threshold:
            return (False, f"Composite FATE violation: {self.composite:.4f} < {threshold}")
...
class Proposal:
    ...
    def compute_results(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "approve_votes": sum(1 for v in self.votes if v.choice == VoteChoice.APPROVE),
            "reject_votes": sum(1 for v in self.votes if v.choice == VoteChoice.REJECT),
            "abstain_votes": sum(1 for v in self.votes if v.choice == VoteChoice.ABSTAIN),
...
class GovernanceHypervisor:
    ...
    async def create_proposal(
        ...
        async with self._gov_lock:
            ...
            voting_deadline = datetime.now(timezone.utc) + (
                timedelta(hours=voting_period_hours) if voting_period_hours
    ...
    async def finalize_proposal(self, proposal_id: str) -> Tuple[bool, str]:
        ...
        async with self._gov_lock:
            ...
            if results["passed"]:
                ...
                if proposal.proposal_type in (ProposalType.PARAMETER_CHANGE, ProposalType.EMERGENCY_ACTION):
                    await self._execute_proposal(proposal)
                
                return (True, f"Proposal PASSED: {results['approval_rate']:.2%} approval")
            else:
                self._transition_proposal(proposal, ProposalStatus.FAILED)
                self._proposals_failed += 1
                return (False, f"Proposal FAILED: {results['approval_rate']:.2%} approval")
    ...
    async def appeal_proposal(
        ...
        async with self._gov_lock:
            ...
            if proposal.status not in (ProposalStatus.PASSED, ProposalStatus.FAILED, ProposalStatus.VETOED):
                return ("", False, f"Cannot appeal proposal in state: {proposal.status.name}")
    ...
    def _transition_proposal(self, proposal: Proposal, new_status: ProposalStatus) -> None:
    ...
    def list_proposals(
        ...
        if proposal_type_filter:
            proposals = [p for p in proposals if p.proposal_type == proposal_type_filter]
...
async def demo_governance_hypervisor():
    ...
    prop1_id, accepted, reason = await hypervisor.create_proposal(
        ...
        fate_metrics=FATEMetrics(fairness=0.98, autonomy=0.96, transparency=0.99, empowerment=0.95),
        ihsan_metrics=IhsanMetrics(ikhlas=0.99, karama=0.97, adl=0.98, kamal=0.96, istidama=0.95),
    )
    ...
    prop2_id, accepted, reason = await hypervisor.create_proposal(
        ...
        ihsan_metrics=IhsanMetrics(ikhlas=0.80, karama=0.85, adl=0.75, kamal=0.90, istidama=0.88),
    )
    ...
    for prop_id in [prop1_id, prop2_id, prop3_id]:
        ...
        if prop.votes:
            ...
            print(f"    Votes: {results['approve_votes']} approve, {results['reject_votes']} reject")
