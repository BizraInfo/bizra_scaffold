...
class IhsanEnforcer:
    ...
    def validate_transaction(self, tx: Transaction) -> Tuple[bool, str]:
        ...
        if tx.ihsan_score < self.IHSAN_THRESHOLD:
            ...
            return (False, f"Ihsan violation: {tx.ihsan_score:.4f} < {self.IHSAN_THRESHOLD}")
    ...
    def _validate_token_transfer(self, tx: Transaction) -> Tuple[bool, str]:
        ...
        if payload.get("recipient_share", 0) > 0.1:  # No single transfer > 10% of supply
    ...
    def _validate_governance(self, tx: Transaction) -> Tuple[bool, str]:
        ...
        if tx.ihsan_score < 0.98:
            return (False, f"Governance requires Ihsan >= 0.98, got {tx.ihsan_score:.4f}")
...
class BlockchainSubstrate:
    ...
    async def create_block(self, proposer: str = None) -> Optional[Block]:
        ...
        async with self._chain_lock:
            ...
            self.pending_transactions = [
                tx for tx in self.pending_transactions
                if tx.status not in (TransactionStatus.COMMITTED, TransactionStatus.REJECTED)
            ]
    ...
    def get_merkle_proof(self, block_index: int, tx_index: int) -> Optional[List[Tuple[bytes, bool]]]:
        """
...
async def demo_blockchain_substrate():
    ...
    transactions = [
        (BlockType.ATTESTATION, "agent_001", {"subject": "code_review", "poi_score": 0.92}, 0.97),
        (BlockType.TOKEN_TRANSFER, "agent_001", {"recipient": "agent_002", "stable_amount": 100}, 0.96),
        (BlockType.STATE_UPDATE, "agent_003", {"state_delta": {"reputation": 0.85}}, 0.99),
        (BlockType.GOVERNANCE, "sat_council", {"action": "update_threshold", "ihsan_threshold": 0.96}, 0.99),
