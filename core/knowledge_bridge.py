...
class KnowledgeGraph:
    ...
    def get_statistics(self) -> GraphStatistics:
        ...
        node_type_dist = {
            nt.name: len(nids)
            for nt, nids in self._nodes_by_type.items()
        }
        ...
        avg_signal = 0.0
        if node_count > 0:
            avg_signal = sum(n.signal_strength for n in self._nodes.values()) / node_count
...
class KnowledgeExtractor:
    ...
    def __init__(self, min_signal_strength: float = 0.3):
        ...
        self._compiled_relationship_patterns = [
            (re.compile(p, re.IGNORECASE), et)
            for p, et in self.RELATIONSHIP_PATTERNS
        ]
            ...
            for match in pattern.finditer(content):
                ...
                if source_node and target_node:
                    edges.append(KnowledgeEdge(
                        ...
                        edge_type=edge_type,
                        weight=(source_node.signal_strength + target_node.signal_strength) / 2,
...
class KnowledgeGraphBridge:
    ...
    async def process_file(self, file_path: Path, snr_score: float = 0.5) -> ExtractionResult:
        """
