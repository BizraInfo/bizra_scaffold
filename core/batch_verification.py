...
class BatchVerificationEngine:
    ...
    async def _process_batch(self) -> BatchResult:
        ...
        batch_id = hashlib.sha256(
            b"".join(a.id.encode() for a in batch)
        ).hexdigest()[:16]
        ...
        for action in batch:
            ...
            action_hashes.append(
                hashlib.sha256(action.payload).digest()
            )
    ...
    def _sign_batch(self, aggregate_hash: str) -> str:
        ...
        if isinstance(self._signing_key, ed25519.Ed25519PrivateKey):
            ...
            import hmac
            sig = hmac.new(
                self._signing_key if isinstance(self._signing_key, bytes) else b"default_key",
                data,
                hashlib.sha256
            ).hexdigest()
            return sig
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "estimated_throughput": avg_batch_size / (avg_time / 1000.0) if avg_time > 0 else 0,
