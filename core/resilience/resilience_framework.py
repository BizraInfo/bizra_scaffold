...
class CircuitBreaker:
    ...
    def __init__(
        ...
        self._state_changes: Deque[Tuple[datetime, CircuitState, CircuitState]] = deque(
            maxlen=1000
        )
...
class Timeout:
    ...
    async def execute(
        ...
        try:
            ...
            raise TimeoutError(f"Operation '{self.name}' timed out after {timeout}s")
    ...
    async def check(self, name: str, timeout: Optional[float] = None) -> HealthStatus:
        ...
        try:
            result = await asyncio.wait_for(
                self._checks[name](), timeout=effective_timeout
            )
    ...
    async def check_all(
        ...
        try:
            results = await asyncio.wait_for(
                asyncio.gather(*[self.check(name) for name in self._checks]),
                timeout=effective_timeout,
            )
        except asyncio.TimeoutError:
            logger.warning(f"check_all timed out after {effective_timeout}s")
...
class ResilienceRegistry:
    ...
    def get_all_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "bulkheads": {name: b.get_metrics() for name, b in self._bulkheads.items()},
            "rate_limiters": {
                name: rl.get_metrics() for name, rl in self._rate_limiters.items()
            },
