    ...
    def from_dict(cls, data: Dict[str, Any]) -> "FileAsset":
        ...
        return cls(
            ...
            last_modified=datetime.fromisoformat(
                data.get("last_modified", datetime.now(timezone.utc).isoformat())
            ),
            last_verified=datetime.fromisoformat(
                data.get("last_verified", datetime.now(timezone.utc).isoformat())
            ),
...
class DataLakeWatcher:
    ...
    def add_watched_path(
        ...
        if not path.exists():
            logger.warning(
                f"Path does not exist: {path}. Will be watched when created."
            )
    ...
    def save_manifest(self, filename: str = "data_lake_manifest.json") -> Path:
        ...
        with self._lock:
            ...
            manifest_checksum = hashlib.sha256("".join(all_hashes).encode()).hexdigest()
        ...
        logger.info(
            f"Manifest saved: {manifest_path} ({len(manifest['assets'])} assets)"
        )
    ...
    def load_manifest(self, filename: str = "data_lake_manifest.json") -> bool:
        ...
        try:
            ...
            logger.info(
                f"Manifest loaded: {len(self.assets)} assets from {manifest_path}"
            )
            return True
    ...
    def score_asset_snr(self, asset: FileAsset) -> float:
        ...
        size_factor = min(
            math.log10(asset.size_bytes + 1) / 7, 1.0
        )  # Cap at 10MB equiv
...
async def run_watcher_cli():
    ...
    parser.add_argument(
        "--watch", action="store_true", help="Start continuous watching"
    )
    parser.add_argument(
        "--interval", type=int, default=60, help="Watch interval (seconds)"
    )
