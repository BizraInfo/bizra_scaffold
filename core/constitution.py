...
class IhsanInvariants:
    ...
    def __post_init__(self):
        ...
        if not 0.0 <= self.minimum_threshold <= 1.0:
            raise ConstitutionValidationError(f"Invalid minimum_threshold: {self.minimum_threshold}")
...
class Constitution:
    ...
    def load(
        ...
        if not TOML_AVAILABLE:
            raise ConstitutionError("toml library required. Install with: pip install toml")
    ...
    def get(cls) -> Constitution:
        ...
        if cls._instance is None:
            raise ConstitutionError("Constitution not loaded. Call Constitution.load() first.")
        return cls._instance
    ...
    def _parse_gates(self) -> Dict[str, GateConfig]:
        ...
        for name, config in gates_raw.items():
            if isinstance(config, dict) and ("order" in config or "id" in config or "tier" in config):
                max_latency = config.get("max_latency_ms", config.get("timeout_ms", 100))
    ...
    def validate_action(
        ...
        if ihsan_score < self._ihsan.minimum_threshold:
            return False, f"Ihsān {ihsan_score} below threshold {self._ihsan.minimum_threshold}"
