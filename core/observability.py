    ...
    def get_layer_breakdown(self) -> Dict[str, Dict[str, float]]:
        ...
        return {
            layer: {
                ...
                "min": min(scores),
                "compliance_rate": sum(1 for s in scores if s >= self._alert_threshold) / len(scores),
            }
...
class TelemetryProvider:
    ...
    def __init__(
        ...
        exporter = (
            CompositeSpanExporter(exporters) if len(exporters) > 1
            else exporters[0]
        )
    ...
    def record_operation(
        ...
        self._operation_counter.add(1, {
            "operation": name,
            "layer": layer,
            "success": str(success),
        })
        
        self._operation_latency.record(duration_ms, {
            "operation": name,
            "layer": layer,
        })
