...
class RateLimiter:
    ...
    def _check_sliding_window(self, key: str) -> RateLimitResult:
        ...
        weighted_count = (
            previous_state.count * (1 - window_position) + current_state.count
        )
    ...
    def _maybe_cleanup(self) -> None:
        ...
        expired_keys = [
            key for key, state in self._buckets.items() if state.last_update < cutoff
        ]
    ...
    def validate(self, value: Any, field: str) -> str:
        ...
        if len(value) < self.min_length:
            raise ValidationError(
                field, f"Must be at least {self.min_length} characters", "min_length"
            )

        if len(value) > self.max_length:
            raise ValidationError(
                field, f"Must be at most {self.max_length} characters", "max_length"
            )

        if self.pattern and not self.pattern.match(value):
            raise ValidationError(field, "Does not match required pattern", "pattern")

        if self.allowed_chars:
            invalid = set(value) - self.allowed_chars
            if invalid:
                raise ValidationError(
                    field, f"Contains invalid characters: {invalid}", "invalid_chars"
                )
...
class NumberValidator(Validator[Union[int, float]]):
    ...
    def validate(self, value: Any, field: str) -> Union[int, float]:
        ...
        if self.min_value is not None and value < self.min_value:
            raise ValidationError(
                field, f"Must be at least {self.min_value}", "min_value"
            )

        if self.max_value is not None and value > self.max_value:
            raise ValidationError(
                field, f"Must be at most {self.max_value}", "max_value"
            )
...
class ListValidator(Validator[List[T]]):
    ...
    def validate(self, value: Any, field: str) -> List[T]:
        ...
        if len(value) < self.min_items:
            raise ValidationError(
                field, f"Must have at least {self.min_items} items", "min_items"
            )

        if len(value) > self.max_items:
            raise ValidationError(
                field, f"Must have at most {self.max_items} items", "max_items"
            )
        ...
        for i, item in enumerate(value):
            try:
                ...
                if self.unique:
                    ...
                    if item_key in seen:
                        raise ValidationError(
                            f"{field}[{i}]", "Duplicate value", "duplicate"
                        )
                    seen.add(item_key)
...
class IhsanScoreValidator(NumberValidator):
    ...
    def __init__(self):
        super().__init__(
            min_value=0.0, max_value=1.0, integer_only=False, positive_only=True
        )
...
class InputSanitizer:
    ...
    def check_sql_injection(self, value: str, field: str) -> None:
        ...
        for pattern in self._sql_patterns:
            if pattern.search(value):
                raise ValidationError(
                    field, "Potential SQL injection detected", "sql_injection"
                )
    ...
    def check_xss(self, value: str, field: str) -> None:
        ...
        for pattern in self._xss_patterns:
            if pattern.search(value):
                raise ValidationError(field, "Potential XSS detected", "xss")
    ...
    def check_path_traversal(self, value: str, field: str) -> None:
        ...
        for pattern in self._path_patterns:
            if pattern.search(value):
                raise ValidationError(
                    field, "Potential path traversal detected", "path_traversal"
                )
    ...
    def _get_cors_headers(self, origin: str) -> Dict[str, str]:
        ...
        headers = {
            "Access-Control-Allow-Origin": (
                ...
                else ("*" if "*" in self.config.cors_allow_origins else origin)
            ),
...
class BIZRASecurityValidators:
    ...
    def validate_value_assessment_request(cls, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            cls.sanitizer.sanitize(validated["convergence_id"], "convergence_id")
        except ValidationError as e:
            errors.append(e)
...
def create_bizra_security_middleware(
    ...
    return {
        ...
        "csrf_protection": CSRFProtection(csrf_secret or secrets.token_urlsafe(32)),
        "api_key_manager": APIKeyManager(api_key_pepper or secrets.token_urlsafe(32)),
