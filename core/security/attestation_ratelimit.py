...
class ReputationTracker:
    ...
    def __init__(
        ...
        self._reputations: LRUCache[str, NodeReputation] = LRUCache(
            max_size=10000, ttl_seconds=86400  # 24 hours
        )
...
class AttestationRateLimiter:
    ...
    def __init__(self, config: Optional[AttestationRateLimitConfig] = None):
        ...
        # Reputation tracking
        self._reputation = (
            ReputationTracker() if self.config.reputation_enabled else None
        )
    ...
    def check_rate_limit(
        ...
        if node_id in self.config.trusted_nodes:
            ...
            return RateLimitResult(
                allowed=True, remaining=999999, reset_after=0.0, limit=999999, current=0
            )
        ...
        op_result = op_limiter.check(user_id=node_id, endpoint=operation.name)
        ...
        # Check global limit
        global_result = self._global_limiter.check(user_id=node_id)
...
class AttestationSecurityMiddleware:
    ...
    async def __call__(
        self, node_id: str, path: str, metadata: Optional[Dict[str, Any]] = None
    ) -> RateLimitResult:
        """
