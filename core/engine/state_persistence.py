...
class DualTokenWallet:
    ...
    owner_id: str
    stable: TokenBalance = field(
        default_factory=lambda: TokenBalance(WalletType.STABLE, 0.0)
    )
    growth: TokenBalance = field(
        default_factory=lambda: TokenBalance(WalletType.GROWTH, 0.0)
    )
    ...
    def _record_tx(
        self, action: str, token_type: WalletType, amount: float, reason: str
    ):
    ...
    def get_total_value(
        self, stable_price: float = 1.0, growth_price: float = 1.0
    ) -> float:
...
class CognitiveState:
    ...
    def update_ihsan(self, dimension: str, score: float) -> None:
        ...
        total = sum(
            self.ihsan_scores.get(dim, 0.95) * weight for dim, weight in weights.items()
        )
    ...
    def add_memory(self, key: str, value: Any, memory_type: str = "working") -> None:
        ...
        if memory_type == "working":
            ...
            self.episodic_memory.append(
                {"key": key, "value": value, "time": time.time()}
            )
...
class AgentState:
    ...
    def __init__(
        ...
        self._transition_history: List[
            Tuple[AgentLifecycleState, AgentLifecycleState]
        ] = []
...
class StatePersistenceEngine:
                ...
                for operation in wallet_updates.get("operations", []):
                    ...
                    if action == "deposit":
                        ...
                        agent.wallet.withdraw(
                            token, amount, operation.get("reason", "")
                        )
                    elif action == "stake":
    ...
    async def create_checkpoint(
        self, agent_id: str, block_height: int
    ) -> AgentCheckpoint:
    ...
    async def migrate_agent(
        ...
        async with self._registry_lock:
            ...
            if not agent.transition_to(AgentLifecycleState.MIGRATING):
                raise ValueError(
                    f"Cannot migrate agent in state {agent.lifecycle_state}"
                )
    ...
    def list_agents(
        self, state_filter: Optional[AgentLifecycleState] = None
    ) -> List[str]:
...
async def demo_state_persistence():
    ...
    for agent_id in engine.list_agents():
        ...
        print(
            f"    Stable: {agent.wallet.stable.amount:.2f} (available: {agent.wallet.stable.available:.2f})"
        )
        print(
            f"    Growth: {agent.wallet.growth.amount:.2f} (staked: {agent.wallet.growth.locked:.2f})"
        )
        print(f"    Ihsan: {agent.cognitive.cumulative_ihsan:.4f}")
