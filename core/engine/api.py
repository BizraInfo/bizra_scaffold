...
def _get_jwt_service() -> SecureJWTService:
    ...
    if _jwt_service is None:
        if not config.jwt_secret:
            raise ValueError(
                "JWT secret not configured. Set BIZRA_JWT_SECRET environment variable."
            )
...
http_requests_total = Counter(
    "bizra_http_requests_total", "Total HTTP requests", ["method", "endpoint", "status"]
)
...
cognitive_cycles_total = Counter(
    "bizra_cognitive_cycles_total", "Total cognitive cycles executed"
)

cognitive_cycle_duration_seconds = Histogram(
    "bizra_cognitive_cycle_duration_seconds", "Cognitive cycle duration in seconds"
)

ihsan_score_gauge = Gauge("bizra_ihsan_score", "Current Ihsān ethical score")

# Layer metrics
l2_compression_ratio = Gauge("bizra_l2_compression_ratio", "L2 LZMA compression ratio")

l3_episode_count = Gauge("bizra_l3_episode_count", "L3 total episodes stored")

l4_node_count = Gauge("bizra_l4_node_count", "L4 semantic graph node count")

# Security metrics
temporal_chain_length = Gauge(
    "bizra_temporal_chain_length", "Length of quantum temporal chain"
)

chain_entropy_total = Gauge(
    "bizra_chain_entropy_total", "Total entropy in temporal chain"
)
...
async def health_check():
    ...
    components = {
        ...
        "l4_graph": (
            "healthy" if app.state.l4 and app.state.l4.driver else "unavailable"
        ),
    }
...
async def execute_cognitive_operation(
    request: CognitiveOperationRequest, user: Dict[str, Any] = Depends(get_current_user)
):
    """
