...
class VirtueEvaluator(OutcomeEvaluator):
    ...
    async def evaluate(self, action: Action, context: Context) -> EthicalEvaluation:
        ...
        # Weighted average
        total_score = sum(
            score * self.VIRTUES[virtue] for virtue, score in virtue_scores.items()
        )

        concerns = [f"Low {v}: {s:.2f}" for v, s in virtue_scores.items() if s < 0.4]
        commendations = [
            f"High {v}: {s:.2f}" for v, s in virtue_scores.items() if s > 0.8
        ]

        reasoning = f"Virtue analysis: " + ", ".join(
    ...
    def _evaluate_temperance(self, action: Action) -> float:
        ...
        balance = 1.0 - abs(benefit_count - harm_count) / (
            benefit_count + harm_count + 1
        )
        return balance
...
class IhsanEvaluator(OutcomeEvaluator):
    ...
    async def evaluate(self, action: Action, context: Context) -> EthicalEvaluation:
        ...
        reasoning = f"Ihsān analysis: " + ", ".join(
...
class ConsequentialEthicsEngine:
    ...
    def _determine_severity(self, score: float, consensus: float) -> VerdictSeverity:
    ...
    def _generate_reasoning_trace(self, evaluations: List[EthicalEvaluation]) -> str:
        ...
        for eval in evaluations:
            lines.append(f"\n{eval.framework.name}:")
            lines.append(
                f"  Score: {eval.score:+.2f} (confidence: {eval.confidence:.0%})"
            )
...
async def self_test():
    ...
    print(
        f"✓ Beneficial action: {verdict1.severity.name}, score={verdict1.overall_score:+.2f}"
    )
    ...
    print(
        f"✓ Mixed action: {verdict2.severity.name}, conditions={len(verdict2.conditions)}"
    )
    ...
    assert (
        verdict3.overall_score < verdict1.overall_score
    ), "Harmful should score lower than beneficial"
    print(
        f"✓ Harmful action: {verdict3.severity.name}, permitted={verdict3.action_permitted}"
    )

    # Test 4: Verify Ihsān integration
    ihsan_eval = next(
        (e for e in verdict1.evaluations if e.framework == EthicalFramework.IHSAN), None
    )
    assert ihsan_eval is not None
    assert ihsan_eval.confidence >= 0.8
    print(
        f"✓ Ihsān integration: score={ihsan_eval.score:+.2f}, confidence={ihsan_eval.confidence:.0%}"
    )
