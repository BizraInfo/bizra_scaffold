    ...
    def from_bytes(cls, data: bytes) -> MerkleProof:
        ...
        for _ in range(path_len):
            ...
            direction = ProofDirection(
                struct.unpack("<B", data[offset : offset + 1])[0]
            )
            offset += 1
...
class RecursiveProofInput:
    ...
    def to_risc0_input(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "receipts": [r.to_circuit_input().hex() for r in self.receipts],
        }
...
class ZKAccumulator:
    ...
    def add(self, receipt: Any) -> int:
        ...
        with self._lock:
            ...
            if len(self._receipts) >= self._max_size:
                raise BatchOverflowError(f"Max batch size {self._max_size} exceeded")
    ...
    def prepare_recursive_input(
        ...
        if not self.is_sealed:
            raise StateError(
                "Accumulator must be sealed before preparing recursive input"
            )
        ...
        return json.dumps(
            {
                ...
                "sealed_state": (
                    self._sealed_state.to_dict() if self._sealed_state else None
                ),
            },
            indent=2,
        )
