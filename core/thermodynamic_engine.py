...
class BIZRAThermodynamicEngine:
    ...
    def create_working_fluid(
        ...
        """
        proposal_hash = hashlib.sha256(str(proposal_data).encode()).hexdigest()[:16]
    ...
    async def run_carnot_cycle(
        ...
        v3 = v2 * (
            self.hot_reservoir.temperature / self.cold_reservoir.temperature
        ) ** (1 / 0.4)
        ...
        second_law_eff = (
            thermal_efficiency / self.carnot_efficiency
            if self.carnot_efficiency > 0
            else 0
        )
    ...
    async def run_otto_cycle(
        ...
        second_law_eff = (
            actual_efficiency / self.carnot_efficiency
            if self.carnot_efficiency > 0
            else 0
        )
        ...
        irreversibility = max(
            0, total_heat_in * (self.carnot_efficiency - actual_efficiency)
        )
        ...
        entropy_production = (
            total_heat_out / self.cold_reservoir.temperature
    ...
    async def run_stirling_cycle(
        ...
        fluid.state = ThermodynamicState.EXHAUST
        process4 = IsochoricProcess(
            self.cold_reservoir.temperature, self.cold_reservoir
        )
        fluid, q4, w4 = process4.execute(fluid)
    ...
    async def run_cycle(
        ...
        **kwargs,
        ...
        fluid_kwargs = {
            k: v for k, v in kwargs.items() if k in ("initial_stake", "urgency")
        }
        fluid = self.create_working_fluid(proposal_data, **fluid_kwargs)
        cycle = cycle_type or self.cycle_type

        if cycle == CycleType.CARNOT:
            return await self.run_carnot_cycle(
                fluid, compression_ratio=compression_ratio
            )
            ...
            return await self.run_stirling_cycle(
                fluid, compression_ratio=compression_ratio
            )
        else:
    ...
    def get_efficiency_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_cycle_time_ms": sum(
                m.cycle_duration_ms for m in self.cycle_history
            )
    ...
    def compute_governance_entropy(
        self, proposal_states: List[Dict[str, Any]]
    ) -> float:
        """
...
async def demo_thermodynamic_governance():
    ...
    for proposal in proposals:
        ...
        print(
            f"  Efficiency: {metrics.thermal_efficiency:.2%} (Carnot: {metrics.carnot_efficiency:.2%})"
        )
