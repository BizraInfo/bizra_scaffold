    ...
    async def check_compliance(
        ...
        if score >= threshold:
            ...
            return (
                False,
                score,
                f"Ihsan score {score:.3f} below threshold {threshold:.3f}",
            )
...
class CommandRouter:
    ...
    def __init__(self):
        self._queues: Dict[Priority, Deque[Command]] = {p: deque() for p in Priority}
...
class NodeZeroCommandCenter:
    ...
    def register_subsystem(self, subsystem: Subsystem) -> None:
        ...
        self.event_bus.subscribe(
            f"health.{subsystem.subsystem_type.name}", health_handler
        )
    ...
    async def execute(self, command: Command) -> CommandResult:
        ...
        compliant, ihsan_score, reason = await self.ihsan_enforcer.check_compliance(
            command
        )
    ...
    async def execute_batch(
        ...
        if len(commands) > MAX_BATCH_SIZE:
            raise ValueError(f"batch size {len(commands)} exceeds max {MAX_BATCH_SIZE}")
    ...
    async def health_check(self) -> SubsystemStatus:
        ...
        return SubsystemStatus(
            ...
            metrics={
                ...
                "watched_paths": (
                    len(self._watcher.watched_paths) if self._watcher else 0
                ),
            },
        )
