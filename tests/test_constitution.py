...
@pytest.mark.skipif(
    not CONSTITUTION_AVAILABLE, reason="Constitution module not available"
)
...
class TestHashVerification:
    ...
    def test_hash_verification_success(self, temp_constitution_file: Path):
        ...
        verified = Constitution.load(
            path=temp_constitution_file, verify_hash=correct_hash
        )
    ...
    def test_hash_verification_failure(self, temp_constitution_file: Path):
        ...
        with pytest.raises(ConstitutionError):
            Constitution.load(path=temp_constitution_file, verify_hash=wrong_hash)
