...
class TestKnowledgeGraph:
    ...
    def populated_graph(self, graph):
        ...
        nodes = [
            ...
            KnowledgeNode(
                id="n3", node_type=NodeType.ENTITY, label="Entity1", signal_strength=0.9
            ),
            KnowledgeNode(
                id="n4", node_type=NodeType.FILE, label="file.py", signal_strength=0.6
            ),
        ]
        for node in nodes:
            graph.add_node(node)
    ...
    def test_add_edge(self, graph):
        ...
        edge = KnowledgeEdge(
            source_id="a", target_id="b", edge_type=EdgeType.RELATED_TO
        )
    ...
    def test_add_edge_missing_nodes(self, graph):
        ...
        edge = KnowledgeEdge(
            source_id="missing1", target_id="missing2", edge_type=EdgeType.RELATED_TO
        )
        result = graph.add_edge(edge)
...
class TestKnowledgeGraphBridge:
    ...
    def test_find_related(self, bridge):
        ...
        # Add nodes and edges
        bridge.graph.add_node(
            KnowledgeNode(id="rel-1", node_type=NodeType.CONCEPT, label="A")
        )
        bridge.graph.add_node(
            KnowledgeNode(id="rel-2", node_type=NodeType.CONCEPT, label="B")
        )
        bridge.graph.add_edge(
            KnowledgeEdge(
                source_id="rel-1",
                target_id="rel-2",
                edge_type=EdgeType.RELATED_TO,
