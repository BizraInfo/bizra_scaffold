...
class TestIhsanPrinciples:
    ...
    def test_verify_fail_closed_on_invalid_input(self):
        ...
        # Test NaN
        dimensions = {"truthfulness": float('nan'), "dignity": 1.0, "fairness": 1.0, 
                      "excellence": 1.0, "sustainability": 1.0}
        passed, score = ihsan.verify(dimensions)
        assert passed is False
        assert score == 0.0
        
        # Test out of range (> 1)
        dimensions = {"truthfulness": 1.5, "dignity": 1.0, "fairness": 1.0,
                      "excellence": 1.0, "sustainability": 1.0}
        passed, score = ihsan.verify(dimensions)
        assert passed is False
        
        # Test negative
        dimensions = {"truthfulness": -0.1, "dignity": 1.0, "fairness": 1.0,
                      "excellence": 1.0, "sustainability": 1.0}
        passed, score = ihsan.verify(dimensions)
        assert passed is False
    ...
    def test_gradient_modulation_factors(self):
        ...
        assert torch.mean(torch.abs(kamal_result)) > torch.mean(torch.abs(istidama_result))
...
class TestHigherOrderLogicBridge:
    ...
    def test_ethical_context_affects_output(self):
        ...
        result_high_ethics = bridge(neural_input.clone(), 
                                    ethical_context={"sensitivity": 0.9})
        
        # Outputs should differ
        assert not torch.allclose(
            result_no_ethics["neural_output"],
            result_high_ethics["neural_output"]
        )
...
class TestPropertyBased:
    ...
    def test_ihsan_score_monotonicity(self):
        ...
        low_dims = {k: 0.5 for k in ["truthfulness", "dignity", "fairness", 
