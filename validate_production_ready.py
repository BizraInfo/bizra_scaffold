...
class ValidationRunner:
    ...
    def validate_ethical_constraints(self) -> bool:
        ...
        try:
            ...
            if "enable_ethical_constraints: bool = True" in content:
                print(
                    f"{Colors.GREEN}✓{Colors.END} Ethical constraints enabled by default"
                )
            else:
                print(
                    f"{Colors.YELLOW}⚠{Colors.END} Ethical constraints may not be enabled by default"
                )
            ...
            print(
                f"{Colors.RED}✗{Colors.END} Error validating ethical constraints: {e}"
            )
    ...
    def run_tests(self) -> bool:
        ...
        try:
            ...
            if allow_unrun:
                ...
                print(
                    f"{Colors.RED}   STRICT MODE: Tests MUST run for production validation{Colors.END}"
                )
    ...
    def validate_configuration(self) -> bool:
        ...
        if req_file.exists():
            ...
            for package in required_packages:
                if package in content.lower():
                    ...
                    print(
                        f"{Colors.YELLOW}⚠{Colors.END} Package may be missing: {package}"
                    )
        ...
        for mfile in monitoring_files:
            ...
            if full_path.exists():
                ...
                print(
                    f"{Colors.YELLOW}⚠{Colors.END} Optional monitoring file missing: {mfile}"
                )
    ...
    def generate_report(self) -> Tuple[bool, str]:
        ...
        if production_ready:
            print(
                f"{Colors.GREEN}{Colors.BOLD}✓ SYSTEM IS PRODUCTION READY{Colors.END}"
            )
            ...
            print(
                f"{Colors.RED}{Colors.BOLD}✗ SYSTEM NOT READY FOR PRODUCTION{Colors.END}"
            )
            print(
...
Environment Variables:
  BIZRA_ALLOW_UNRUN_TESTS=1  Same as --allow-unrun-tests
    ...
    parser.add_argument(
        "--allow-unrun-tests",
        action="store_true",
        default=os.environ.get("BIZRA_ALLOW_UNRUN_TESTS", "").lower()
        in ("1", "true", "yes"),
