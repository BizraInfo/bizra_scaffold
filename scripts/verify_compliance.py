...
def main():
    ...
    section3 = (
        "0.95" in sot.split("## 3. Invariants")[1].split("## 4.")[0]
        if "## 3. Invariants" in sot
        else False
    )
    section4 = (
        "0.95" in sot.split("## 4. PoI Parameters")[1].split("## 5.")[0]
        if "## 4. PoI Parameters" in sot
        else False
    )
    ...
    null_evidence = sum(
        1
        for c in claims
        if c.get("status") == "VERIFIED" and not c.get("evidence_artifact_path")
    )
