...
class Dashboard:
    ...
    def print_header(self) -> None:
        ...
        print(
            self.box.middle(f"{Colors.DIM}              v{self.VERSION}{Colors.RESET}")
        )
    ...
    def render_status(
        ...
        close_line = (
            self.light_box.bl
            + self.light_box.h * (self.config.width - 6)
            + self.light_box.br
        )
        ...
        while self._running:
            try:
                ...
                command = await loop.run_in_executor(
                    None, lambda: input(f"{Colors.CYAN}bizra{Colors.RESET}> ")
                )
...
Examples:
  ...
  python dashboard_cli.py search "concept"   # Search knowledge graph
    ...
    parser.add_argument(
        "command", nargs="?", help="Command to execute (interactive mode if omitted)"
    )
