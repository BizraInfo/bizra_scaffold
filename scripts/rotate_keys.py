...
class KeyRotationManager:
    ...
    def generate_key_pair(self, role: str, validity_days: int = DEFAULT_KEY_VALIDITY_DAYS) -> KeyPair:
        """
    ...
    def store_key_pair(self, key_pair: KeyPair, set_active: bool = True) -> str:
        ...
        event = RotationEvent(
            ...
            new_key_id=key_id,
            reason="Key generation" if not key_pair.metadata.predecessor_id else "Key rotation",
    ...
    def list_keys(self, role: Optional[str] = None, include_revoked: bool = False) -> list:
        ...
        for key_id, meta in self.registry["keys"].items():
            ...
            if expires < datetime.now(timezone.utc) and meta["status"] not in ("revoked", "expired"):
                meta["status"] = "expired"
...
def main():
    ...
    parser.add_argument(
        "--role",
        choices=["pat", "sat"],
        help="Agent role for key operation"
    )
    parser.add_argument(
        "--output",
        default="keys/",
    ...
    parser.add_argument(
        "--rotate",
        action="store_true",
        help="Rotate the active key"
    )
    parser.add_argument(
        "--revoke",
        metavar="KEY_ID",
        help="Revoke a specific key"
    )
    parser.add_argument(
        "--reason",
    ...
    parser.add_argument(
        "--list-active",
        action="store_true",
        help="List active keys"
    )
    parser.add_argument(
        "--list-all",
        action="store_true",
        help="List all keys including revoked"
    )
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--constitution",
        default="constitution.toml",
        help="Path to constitution.toml"
    )
    ...
    if args.generate:
        ...
        if not args.force:
            confirm = input(f"⚠️ REVOKE key {args.revoke}? This is IRREVERSIBLE! [y/N]: ")
            if confirm.lower() != "y":
                print("Aborted.")
                sys.exit(0)
