...
async def validate_ihsan_enforcement() -> bool:
    ...
    if result_low_ihsan.level == SNRLevel.HIGH:
        ...
        raise AssertionError("CRITICAL: Ethical override failed - HIGH SNR with low Ihsān!")
    ...
    if result_high_ihsan.level != SNRLevel.HIGH:
        ...
        raise AssertionError(f"Ihsān enforcement too strict - got {result_high_ihsan.level.name}")
    ...
    if not default_scorer.enable_ethical_constraints:
        ...
        raise AssertionError("SNRScorer must have enable_ethical_constraints=True by default")
