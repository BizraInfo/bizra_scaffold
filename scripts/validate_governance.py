...
def validate_evidence_index():
    ...
    row_pattern = re.compile(r"^\|\s*(EVID-\d+)\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|$")
    ...
    for i, line in enumerate(lines):
        ...
        if match:
            ...
            if status == "PENDING":
                ...
                verified_count += 1
                if "pending" in evidence_artifact.lower() or "<commit>" in evidence_artifact:
                    print(f"FAIL: {evidence_id} is VERIFIED but has placeholder artifact: {evidence_artifact}")
                    rows_valid = False
...
if __name__ == "__main__":
    ...
    if not os.path.exists("EVIDENCE_INDEX.md") and os.path.exists("../EVIDENCE_INDEX.md"):
        os.chdir("..")
